...
SIZE_X = int(settings[0])
SIZE_Y = int(settings[1])
seed = int(settings[2])
amt = int(settings[3])
STATE_DEFAULT = 0
STATE_CLICKED = 1
...
window = None
def random_coord(amount, source, xsize, ysize):
    # 设置随机数种子，使得参数一致时结果一致
    random.seed(source)
    # 创建一个空列表，用于存储坐标
    coord = []
    # 循环amount次，每次生成一个随机坐标
    for tt in range(amount):
        # 生成x和y坐标，范围为0到100
...
class Minesweeper:
    ...
    def setup(self):
        ...
        for x in range(0, SIZE_X):
            for y in range(0, SIZE_Y):
                ...
                if (x, y) in random_coord(int(amt), int(seed), int(SIZE_X), int(SIZE_Y)):
                    isMine = True
                    self.mines += 1
    ...
    def gameOver(self, won):
        ...
        msg = "You Win!\nTime used:"+str(datetime.now() - self.startTime)+'s'+'\nboard:'+str(SIZE_X)+'x'+str(SIZE_Y)+'\nNumber'\
                                                                          ' of mines:'+str(amt)+'\nGame code:'+str(seed)+"\nPlay again?" if won\
            else "You Lose! Play again? "
